...
class PJLinkClient:
    def __init__(self, host: str, port: int = 4352, password: str = "", timeout: float = 8.0, retries: int = 4):
        self.host = host
        self.port = port
        ...
        self.input_map = {"HDMI1": "31", "HDMI2": "32", "AUTO": "A0"}
    ...
    async def _handshake(self, r: asyncio.StreamReader, w: asyncio.StreamWriter):
        ...
        banner = await asyncio.wait_for(r.readline(), self.timeout)
    ...
    async def _send_cmd(self, cmd: str) -> str:
        ...
        for attempt in range(self.retries + 1):
            try:
                ...
                resp = await asyncio.wait_for(r.readline(), self.timeout)
                w.close()
                try:
                    await w.wait_closed()
                except Exception:
